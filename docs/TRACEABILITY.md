...
| `MET-031` <sup>N</sup> | An interactive terminal UI (`cdm migrate --tui`) MUST show live throughput, progress bar, ETA, per-node status in cluster… | `cdm-metrics` (`dashboard`), `cdm-cli` (`tui`, `harness::observe`), `cdm-cql` (`connect::session::ClusterSession::nodes`) | `met_031_*` | #39 |
