...
| `MET-010` <sup>N</sup> | Rows and bytes per second per side over 1s/10s/60s EWMA windows, request-latency percentiles per side and operation, in-flight requests, batch sizes, retries by cause, rate-limiter wait time, ranges by state and an ETA | `cdm-metrics` (`instrument`, `progress`), `cdm-core` (`observe`), `cdm-cql` (`observe`, `exec`, `rows`), `cdm-engine` (`scheduler::limits`, `scheduler::ratelimit`), `cdm-cli` (`harness::build`, `harness::mod`) | `met_010_*` | #36, #36a |
...
| `MET-031` <sup>N</sup> | An interactive terminal UI (`cdm migrate --tui`) MUST show live throughput, progress bar, ETA, per-node status in cluster… | `cdm-metrics` (`dashboard`), `cdm-cli` (`tui`, `harness::observe`), `cdm-cql` (`connect::session::ClusterSession::nodes`) | `met_031_*` | #39, #36a |
...
| `TST-020` | **Differential tests against Java CDM**: both implementations run over one seeded corpus — every CQL type, nesting depth 3, nulls, empty collections, edge-case values — and must produce byte-identical target state and identical counter blocks. Nightly in CI, and a gate: a difference exits non-zero and uploads the diff | `cdm-testkit::differential` (feature `differential`), `cdm-testkit / tests/differential`, `xtask` (`cargo xtask differential`), `bench/java-comparison/environment/**`, `.github/workflows/differential.yml` | `tst_020_*` | #34 |
...
| `TST-060` | **Benchmarks**: `criterion` micro-benchmarks for the hot path (bind, convert, compare) and a reproducible macro-benchmark… | `crates/*/benches`, `cdm-testkit::macrobench`, `cdm-testkit / tests/macrobench.rs`, `xtask` (`cargo xtask bench`) | `tst_060_*` | #55 |
