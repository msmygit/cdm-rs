...
| #55 | **Partially delivered.** Tier 1: `criterion` micro-benchmarks over the hot path (`TST-060`), and `bench.yml` restored to a nightly schedule with a `bench` opt-in label. Tier 2: the containerised macro-benchmark, `cargo xtask bench`, plus a weekly `bench.yml` job that records end-to-end rows per second into its own trend series. That job is deliberately **not** a gate — throughput on a shared runner tracks the runner, not the commit. Two things remain, and `docs/BENCHMARKS.md` says so rather than implying otherwise. First, `NFR-004`'s ≥ 2× claim is still unmeasured: tier 2 measures cdm-rs against itself, while the claim is against Java CDM, a Spark application, so the comparison needs Spark, both clusters and dedicated hardware — a written-up exercise rather than a CI job. Second, `TST-060`'s "regressions > 10% MUST fail CI" is **not met** — wall-clock variance on a shared `ubuntu-latest` runner is routinely 10–30%, so a 10% threshold detects noise, not regressions. The gate ships at 200%, which catches lost fast paths and accidental quadratics; closing the gap honestly needs instruction-count benchmarking (`iai-callgrind`). |
