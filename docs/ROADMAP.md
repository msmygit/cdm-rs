...
| #55 | **Partially delivered.** `criterion` micro-benchmarks over the hot path (`TST-060`), and `bench.yml` restored to a nightly schedule with a `bench` opt-in label. Two things are deliberately *not* done, and `docs/BENCHMARKS.md` says so rather than implying otherwise. First, `NFR-004`'s ≥ 2× claim is still unmeasured: Java CDM is a Spark application, so the comparison needs Spark, both clusters and dedicated hardware, which is a written-up exercise rather than a CI job. Second, `TST-060`'s "regressions > 10% MUST fail CI" is **not met** — wall-clock variance on a shared `ubuntu-latest` runner is routinely 10–30%, so a 10% threshold detects noise, not regressions. The gate ships at 200%, which catches lost fast paths and accidental quadratics; closing the gap honestly needs instruction-count benchmarking (`iai-callgrind`). |
...
| ~~`bench.yml`~~ | ~~#55~~ — **live** | restored by #55 to a nightly schedule plus an opt-in `bench` label on pull requests; a full workspace bench build is far too slow for every push |
