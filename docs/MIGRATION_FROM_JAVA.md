...
| 40 | `AstraDevOpsClient.downloadBundleFile` logs the bundle's download URL at `INFO`: `Downloading secure bundle from URL: {}` | The URL is never logged, at any level (`SEC-001`, `CON-004`) | That URL is a pre-signed link to a zip containing the database's **private key**. Anyone who can read the log can download the bundle until the signature expires, without the token. The database id, region and bundle type are still logged, which is what an operator actually needs in order to tell which bundle was fetched | — |
| 41 | With no `scb.region` configured, `AstraDevOpsClient.extractDownloadUrl` takes `rootNode.get(0)` -- the first datacenter in the response array | The **primary** datacenter, identified by `datacenterID` ending in `-1`, wherever it appears in the array (`CON-004`) | Astra documents no ordering for that array, so first-is-primary is an assumption rather than a contract. On a multi-region database where it does not hold, Java downloads a secondary region's bundle and connects there: every request then crosses regions, and the symptom is latency rather than an error. A response with no `datacenterID` still falls back to the first entry, so a single-region database behaves exactly as before | set `connect.{side}.astra.region` explicitly, which overrides both |
| 42 | A `DiffData` run that found mismatches or missing rows exits `0`. Nothing in `src/main/scala` calls `System.exit`, so the `spark-submit` process succeeds unless the job throws | Exit `1` — "completed with discrepancies" (`CLI-004`) | A pipeline that gates a cutover on a validation run can see only the exit code, and a validation whose whole purpose is to answer "did everything arrive" must not answer it by succeeding either way. The counter block is unchanged, so a script that parses `Final Mismatch Record Count` keeps working. Item 33 says the same thing about the guardrail; this is the validate half, found by porting `smoke/04_counters`, whose `fix` step deliberately leaves one counter row uncorrected | read the counter block instead of the code |
| 43 | `CqlTable.formatName` returns any function form unchanged (`return name; // function`), so the identifier inside `TTL(col)` / `WRITETIME(col)` is never quoted | The argument is formatted like any other identifier, so a reserved-word or case-sensitive column becomes `TTL("order")` (`SCH-002`, `SCH-007`) | The same column is quoted in the projection list and bare inside the function, so the server is sent `SELECT col1,"order",TTL(order)` and rejects the whole statement with a syntax error. Java therefore cannot preserve TTL or writetime on a column whose name is reserved or case-sensitive; the run fails at prepare time rather than producing wrong data | — |
